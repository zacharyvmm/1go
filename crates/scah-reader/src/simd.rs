...
// ── PSHUFB nibble-lookup tables (simdjson-style) ───────────────────────
//
// Generated by docs/backtracking_table_gen.rs for scah's boundary bytes:
//   structural: " (0x22)  ' (0x27)  = (0x3D)  > (0x3E)
...
//   bit 0 (0x01) → quote  (" and ')
//   bit 1 (0x02) → equals (=) and gt (>)
//   bit 2 (0x04) → whitespace (tab, LF, FF, CR)
//   bit 3 (0x08) → space
...
///
/// Since only ~1% of bytes are boundaries, this runs only on hit positions
/// and costs a single branch tree / jump table.
#[inline]
pub fn classify_boundary_byte(b: u8) -> BoundaryKind {
    match b {
        ...
        _ => {
            debug_assert!(false, "PSHUFB false positive for byte 0x{b:02X}");
            // Safe fallback (unreachable in correct table configurations)
            BoundaryKind::Whitespace
        }
    }
}

// ── PSHUFB-based combined boundary scanner ─────────────────────────────

/// Scan 32 bytes via PSHUFB nibble lookup.
///
/// Returns a `u32` bitmask where each set bit (bit 7 per byte) marks
/// a boundary character.  Category is determined later via
/// [`classify_boundary_byte`].
///
/// # Safety
/// The caller must ensure AVX2 is available.
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn scan_boundary_combined_avx2(input: std::arch::x86_64::__m256i) -> u32 {
    unsafe {
        ...
        // Extract high nibbles: srli_epi16 shifts 16-bit lanes, so adjacent
        // bytes cross-contaminate.  Mask with 0x0F0F to isolate the result.
        let nibbles = std::arch::x86_64::_mm256_and_si256(
            std::arch::x86_64::_mm256_srli_epi16(input, 4),
            std::arch::x86_64::_mm256_set1_epi16(0x0F0F),
        );
        let hi = std::arch::x86_64::_mm256_shuffle_epi8(hi_table, nibbles);

        let combined = std::arch::x86_64::_mm256_and_si256(lo, hi);

        // `combined` has bits 0–3 set for boundaries.  Collapse to bit 7
        // so movemask can detect them.
        let zero = std::arch::x86_64::_mm256_setzero_si256();
        let not_boundary = std::arch::x86_64::_mm256_cmpeq_epi8(combined, zero);
        let in_bit7 =
            std::arch::x86_64::_mm256_andnot_si256(not_boundary, std::arch::x86_64::_mm256_set1_epi8(0x80u8 as i8));

        std::arch::x86_64::_mm256_movemask_epi8(in_bit7) as u32
    }
}

/// Scan 16 bytes via NEON PSHUFB-like tbl lookup.
///
/// # Safety
/// The caller must ensure NEON is available.
#[cfg(target_arch = "aarch64")]
...
unsafe fn scan_boundary_combined_neon(input: std::arch::aarch64::uint8x16_t) -> u32 {
    unsafe {
        ...
        // Collapse any non-zero byte to bit 7 for movemask
        let zero = std::arch::aarch64::vdupq_n_u8(0);
        let is_boundary = std::arch::aarch64::vceqq_u8(combined, zero); // 0xFF where zero
        let in_bit7 = std::arch::aarch64::vmvnq_u8(is_boundary); // invert: 0xFF → 0x00, 0x00 → 0xFF
        let in_bit7 = std::arch::aarch64::vandq_u8(in_bit7, std::arch::aarch64::vdupq_n_u8(0x80));
    ...
    unsafe {
        ...
        while pos + 32 <= len {
            ...
            if mask != 0 {
                ...
                let abs_pos = pos + bit;
                let byte = input[abs_pos];
                return Some(BoundaryHit {
                    position: abs_pos,
                    kind: classify_boundary_byte(byte),
                });
            }
            pos += 32;
        }
