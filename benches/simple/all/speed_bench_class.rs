...
fn generate_html(count: usize) -> String {
    ...
    for i in 0..count {
        html.push_str(&format!(
            r#"<div class="article"><a class="link" href="/post/{i}"><b>Post</b> &lt;{i}&gt;</a></div>"#,
            i = i
        ));
    }
...
fn bench_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("simple_all_class_selection");
            ...
            |b, _| {
                b.iter(|| {
                    let query = Query::all(black_box(QUERY), Save::all())
                        .expect("class bench selector should parse")
                        .build();
                    black_box(query);
                })
            },
