...
//!
//! - `linear_eager`: scan one tag and eagerly parse its attributes immediately.
//! - `linear_lazy`: scan one tag, reject by name, then parse attributes.
...
fn build_tape_dense_auto_simd(html: &str, classes: &mut Vec<u8>, tape: &mut Vec<ElementSpan>) {
    ...
    for (position, &class) in classes.iter().enumerate() {
        ...
        if class & CLASS_DOUBLE_QUOTE != 0 && !matches!(state, DenseState::Text) {
            quote = CLASS_DOUBLE_QUOTE;
            continue;
        }
        if class & CLASS_SINGLE_QUOTE != 0 && !matches!(state, DenseState::Text) {
...
fn linear_eager(html: &str, selector: SelectorCase) -> MatchSummary {
...
fn linear_lazy(html: &str, selector: SelectorCase) -> MatchSummary {
    let mut summary = MatchSummary::default();
    let mut cursor = 0;
    while let Some(span) = next_element_span(html, &mut cursor) {
...
fn linear_lazy_memchr_simd(html: &str, selector: SelectorCase) -> MatchSummary {
    let mut summary = MatchSummary::default();
    let mut cursor = 0;
    while let Some(span) = next_element_span_memchr_simd(html, &mut cursor) {
        if span.is_close() || !name_matches(span.name(html), selector) {
...
fn bench_element_tape(c: &mut Criterion) {
    ...
    for rows in [100, 1_000, 10_000] {
        ...
        for selector in SELECTORS {
            let linear = linear_eager(&html, selector);
            let linear_lazy_result = linear_lazy(&html, selector);
            let linear_lazy_memchr_simd_result = linear_lazy_memchr_simd(&html, selector);
            let eager = tape_eager(&html, &index_tape, selector);
            let lazy = tape_lazy(&html, &index_tape, selector);
            assert_eq!(linear.matches, linear_lazy_result.matches);
            assert_eq!(linear.matches, linear_lazy_memchr_simd_result.matches);
            assert_eq!(linear.matches, eager.matches);
            assert_eq!(linear.matches, lazy.matches);
            ...
            let production_matches = parse(&html, validation_queries)
                ...
                .unwrap()
                .count();
            assert_eq!(linear.matches, production_matches);

            let parameter = format!("{}/{rows}", selector.label);
            group.bench_with_input(
                BenchmarkId::new("linear_eager", &parameter),
                &html,
                |b, html| b.iter(|| black_box(linear_eager(black_box(html), selector))),
            );

            group.bench_with_input(
                BenchmarkId::new("linear_lazy", &parameter),
                &html,
                |b, html| b.iter(|| black_box(linear_lazy(black_box(html), selector))),
            );

            group.bench_with_input(
                BenchmarkId::new("linear_lazy_memchr_simd", &parameter),
                &html,
                |b, html| b.iter(|| black_box(linear_lazy_memchr_simd(black_box(html), selector))),
            );
...
criterion_group!(benches, bench_element_tape, bench_sparse_index_policy);
